...
import { DollarSign, LayoutList, Users } from "lucide-react";
...
const calculateBadgeProgress = (totalPoints) => {
  ...
  if (totalPoints >= goldThreshold) {
    ...
    silverProgress = 100;
    goldProgress =
      ((totalPoints - silverThreshold) / (goldThreshold - silverThreshold)) *
      100;
  } else if (totalPoints >= bronzeThreshold) {
    bronzeProgress = 100;
    silverProgress =
      ((totalPoints - bronzeThreshold) / (silverThreshold - bronzeThreshold)) *
      100;
    goldProgress = 0;
  } else {
...
const HeaderSection = React.memo(
  ({ userInfo, balance, totalEarnings, isVerified }) => (
    <div
      style={{
...
const StatCards = React.memo(({ stats, userInfo, XUSDBalance }) => {
  const statCardItems = [
    {
      icon: <DollarSign />,
      ...
      value:
        userInfo?.isVerified === true
          ? `$${stats?.currentBalance?.toFixed(2) ?? 0}`
          : `${formatPrice(userInfo?.escrowBalance)}`,
    },
    {
      icon: <DollarSign />,
      title: "XUSD",
      value: `$${XUSDBalance?.toFixed(2) || "0.00"}`,
    },
    {
      icon: <LayoutList />,
      ...
      value:
        userInfo?.isVerified === true
          ...
          : `${userInfo?.tasksCreated.length} Created`,
    },
    // {
    // 	icon: <LayoutList />,
    // 	title: "Success Rate",
...
const MainContent = React.memo(
  ({ stats, pieChartData, recentWithdrawals, recentDeposits, userInfo }) => (
    <div className="flex flex-wrap w-full gap-6">
      {userInfo?.isVerified === false && (
        <motion.div
          ...
          transition={{ duration: 0.5 }}
          className="rounded-lg w-full border-gray-300/55"
        >
          <FreelancerEarningsChart stats={stats} />
        </motion.div>
      )}
      {userInfo?.isVerified === true && (
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:col-span-2 items-center border w-full">
          <motion.div
            initial={{ scale: 0, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            transition={{ duration: 0.5 }}
            className="rounded-lg border-gray-300/55"
          >
            <FreelancerEarningsChart stats={stats} />
          </motion.div>
          <motion.div
            ...
            animate={{ scale: 1, opacity: 1 }}
            transition={{ duration: 0.5 }}
            className="rounded-lg border-gray-300/55 bg-white"
          >
      ...
      >
        <div className="border rounded-lg border-gray-300/55 p-4">
          <DataTable data={stats?.allOrders ?? []} columns={columns} />
        </div>
      </motion.div>
    </div>
  )
...
const StatCard = React.memo(({ icon, title, value }) => (
  <div className="p-5 col-span-1 bg-gray-50 shadow-lg rounded-lg space-y-4">
    <div className="flex gap-2 items-center">
      <div className="p-2 shadow rounded-full">{icon}</div>
