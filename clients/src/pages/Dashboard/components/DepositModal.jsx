...
function DepositModal({ showDepositModal, setShowDepositModal, reloadData }) {
  const { showToast, dismissToast, loadingToast, successToast, errorToast } =
    useShowToast();
  ...
  useEffect(() => {
    ...
    if (isDepositConfirmed) {
      successToast(`${amount} XUSD deposit successful!`, { id: toastId });
      refetch();
  ...
  useEffect(() => {
    if (isXUSD && amount && isConnected) {
      checkApproval();
    }
  }, [isXUSD, amount, isConnected]);
  ...
  const handleAmountChange = (e) => {
    ...
    if (isNaN(value) || parseFloat(value) <= 0) {
      showToast(
        "Error",
        "Please enter a valid amount greater than $0.50",
        "error"
      );
      setAmount("");
    } else {
  ...
  const handleXUSDDeposit = async () => {
    ...
    try {
      ...
      const approvalTx = await approveXUSD(amount);
      ...
      if (isFundPlatformComplete) {
        const newBalance = user.XUSDBalance + amount;
        const updatedUser = { ...user, XUSDBalance: newBalance };
        setUser(updatedUser);
        // reloadData();
      }
      console.log(userId, amount);
    } catch (error) {
      console.error("Error in handleXUSDDeposit:", error);
  ...
  console.log(XUSDBalance?.toFixed(2));
  return (
              ...
              <div className="flex-col gap-2">
                ...
                <div className="flex justify-end gap-1 mt-4">
                  ...
                  {!isXUSD ? (
                    ...
                    users?.paymentWallet ||
                    users?.paymentWalletRegisterComplete === true ? (
                      <Button
                        colorScheme="blue"
                        size={["sm", "md"]}
                        onClick={handleXUSDDeposit}
                        isDisabled={
                          !amount ||
                          isDepositing ||
                          isFundPlatformPending ||
                      ...
                      <RegisterUserBtn label={"Register"} />
                    )
                  ) : (
